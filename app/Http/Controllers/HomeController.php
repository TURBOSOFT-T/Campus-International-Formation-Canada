...
use App\Models\{Blog, BlogView, commandes,config,Coach, User,produits, Category, Service,Marque, Video,Event, Formation, Sponsor,Image, Offre};
...
class HomeController extends Controller
{
    ...
    public function home(Request $request)
    {
     ...
     //  $produits = produits::select('*')->latest()->take(20)->get();
       ...
       $configs= config::all();
       $banners = Banners::select("titre","sous_titre","image")->latest()->get();
 ...
 */
    ...
    {
        $categories = Category::withCount('offres')
        ->has('offres')
        ->get();
      
       $lasts = Offre::select('*')->latest()->take(5)->get();
       
        $offres = Offre::select('*')->latest()->get();
        
        return view('front.offres.index', compact( 'categories', 'offres','lasts'));
    }


      public function offresByCategory($id)
    {
        $categories = Category::withCount('offres')
        ->has('offres')
        ->get();
      
        $current_category = Category::has('offres')
    ->findOrFail($id);

     $offres = $current_category->offres()->paginate(10);
       $lasts = Offre::select('*')->latest()->take(5)->get();
    ...
    public function details_offres($id){
        $blog =Offre:: findOrFail($id);
        $lasts = Offre::select('*')->latest()->take(5)->get();
    $categories = Category::withCount('offres')
        ->has('offres')
        ->get();
        $configs= config::all();
        return view('front.offres.details', compact('blog','configs', 'lasts', 'categories'));
    }

    public function searchoffre(SearchRequest $request)
    {
        $search = $request->search;
        $lasts = Offre::select('*')->latest()->take(5)->get();
        $categories = Category::has('offres')->get();
        $offres = Offre::where('nom', 'like', '%'.$search.'%')
            ->orWhere('description', 'like', '%'.$search.'%')
            ->paginate(10);
        $title = __('Actualité nont trouvée avec la recherche: ') . '<strong>' . $search . '</strong>';
     
        return view('front.offres.index', compact('offres', 'title','categories', 'lasts'));
    }
      ...
      public function servicesByCategory($id)
    {
      ...
      //  $current_category = Category::with('services','images')->findOrFail($id);
        ...
        $services = $current_category->services()->get();
        $imgs = $current_category->images()->get();
        return view('front.services.index', compact('current_category', 'services','imgs'));
    }
    ...
    $configs= config::all();
      ...
      public function details_services($id){
        ...
        $services = Service::all();
         $imgs = $service->images()->get();
        return view('front.services.details', compact('current_category','service','services','imgs'));
      }
